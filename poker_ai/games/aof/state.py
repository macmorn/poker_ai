...
from poker_ai.poker.engine import AoFPokerEngine
...
def new_game(
    ...
    pot = Pot()
    players = [
        AOFPokerPlayer(player_i=player_i, initial_chips=800, pot=pot)
        for player_i in range(n_players)
    ]
    if use_lut == False:
        # Don't reload massive files, it takes ages.
        state = AOFPokerState(players=players, use_lut=False, **kwargs)
        state.card_info_lut = card_info_lut
    else:
        # Load massive files.
        state = AOFPokerState(players=players, **kwargs)
    return state
...
def new_random_game(
    ...
    """
    Create a new random game of AoF poker.
    ...
    card_info_lut : InfoSetLookupTable
        Card information cluster lookup table.
    total_pot: int
        Pot to be split amongst players.
    ...
    players = [
        AOFPokerPlayer(player_i=player_i, initial_chips=chip_nums[player_i], pot=pot)
        for player_i in range(n_players)
    ]
    if use_lut == False:
        # Don't reload massive files, it takes ages.
...
class AOFPokerState:
    """The state of a AoF Poker game at some given point in time.

    The class is immutable and new state can be instanciated from once an
    action is applied via the `AOFPokerState.new_state` method.
    """

    def __init__(
        self,
        players: List[AOFPokerPlayer],
        small_blind: int = 50,
        big_blind: int = 100,
        lut_path: str = ".", 
        pickle_dir: bool = False,
        use_lut: bool = False,#if False passed hand group eval switches to eval based
    ):
        """Initialise state."""
        ...
        self._table = PokerTable(
            players=players, pot=players[0].pot, include_ranks=range(2,15)
        )
        # Get a reference of the initial number of chips for the payout.
        self._initial_n_chips = [p.n_chips for p in players]
        self.small_blind = small_blind
        self.big_blind = big_blind
        self._poker_engine = AoFPokerEngine(
    ...
    def __repr__(self):
        ...
        return f"<AOFPokerState player_i={self.player_i} betting_stage={self._betting_stage}>"

    def apply_action(self, action_str: Optional[str]) -> AOFPokerState:
        ...
        action_str : str or None
            ...
            if the agent hasn't folded already.x

        Returns
        -------
        new_state : AOFPokerState
        ...
        if action_str is None:
            ...
            raise_n_chips = new_state.current_player.n_chips
    ...
    @property
    def private_hands(self) -> Dict[AOFPokerPlayer, List[Card]]:
    ...
    def payout(self) -> Dict[int, int]:
        ...
        for player_i, player in enumerate(self.players):
            n_chips_delta[player_i] = player.n_chips - self._initial_n_chips[player_i]#TODO: potential bugfix, lets see if it changes smth
    ...
    @property
    def players(self) -> List[AOFPokerPlayer]:
        """Returns players in table."""
        return self._table.players

    @property
    def current_player(self) -> AOFPokerPlayer:
